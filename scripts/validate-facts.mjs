...
import { assertNoYearInTitles, titleCatalogueSize, titleEnFor, titlePtFor } from './lib/eventText.mjs'
...
const ids = new Set(), titles = new Set(), queries = new Set(), categories = new Set()
const errors = []
...
for (const fact of facts) {
  ...
  if (!english || normalize(english) === query) errors.push(`Título em inglês ausente no catálogo: ${fact.id}`)
  try { assertNoYearInTitles(fact) } catch (error) { errors.push(error.message) }
}

// Every round deals one card per category, so the pool needs at least four of them.
if (categories.size < ROUND_SIZE) errors.push(`Rodadas precisam de ${ROUND_SIZE} categorias; o acervo tem ${categories.size}.`)

if (titleCatalogueSize !== facts.length) {
  errors.push(`Catálogo de títulos em inglês tem ${titleCatalogueSize} entradas para ${facts.length} fatos.`)
}
...
console.log(`OK: ${facts.length} fatos em ${categories.size} categorias, IDs/títulos/consultas únicos, anos válidos e referência preenchida.`)
