...
function LeaderboardRow({ row, index, isYou }) {
  const { t } = useI18n()
  return (
    <div className={`leaderboard-row ${isYou ? 'is-you' : ''}`} style={{ '--row-delay': `${index * ROW_STAGGER_MS}ms` }}>
...
export default function LeaderboardPage() {
  ...
  let content
  if (loading) content = <div className="center-loader"><LoaderCircle className="spin" /></div>
  else if (!rows.length) content = <p className="empty-state">{t.noLeaderboard}</p>
  else {
    content = (
      <div className="leaderboard-list">
        {rows.map((row, index) => <LeaderboardRow key={row.id} row={row} index={index} isYou={row.id === user?.uid} />)}
      </div>
    )
  }
